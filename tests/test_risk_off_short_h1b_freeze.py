...
from src.strategy.freeze_risk_off_short_h1b import DEFAULT_STRATEGY_SPEC_PATH, build_h1b_fold_thresholds


def test_h1b_strategy_spec_yaml_validates() -> None:
    spec = StrategySpec.from_yaml(DEFAULT_STRATEGY_SPEC_PATH)

    assert spec.strategy_id == "qqq_15min_risk_off_short_h1b_v1"
    assert spec.target_symbol == "QQQ"
    assert spec.position.side == "short_only"
    assert spec.exit_rule.horizon_bars == 6


def test_build_h1b_fold_thresholds_freezes_train_fit_values() -> None:
    frame = pd.DataFrame(
        {
            ...
            "spread_credit_12": [-0.5, -1.5, -2.0, -3.0],
            "hour": [10, 11, 12, 13],
        }
    )
    fold = ResearchFold(
    ...
    assert row["credit_weak_q50_threshold"] == -1.0
    assert row["validation_sessions"] == 1
