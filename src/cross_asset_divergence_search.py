...
VARIANT_FLAGS: dict[str, dict[str, bool]] = {
    "breadth_nonconfirm": {"breadth": True, "credit": False, "relative": False},
    "credit_nonconfirm": {"breadth": False, "credit": True, "relative": False},
    "breadth_credit_nonconfirm": {"breadth": True, "credit": True, "relative": False},
    "relative_leadership_fade": {"breadth": False, "credit": False, "relative": True},
    "relative_breadth_credit": {"breadth": True, "credit": True, "relative": True},
}
...
def _search_cfg(config: dict[str, Any]) -> dict[str, Any]:
    return config.get("cross_asset_divergence_search", {})


def report_output_path(config: dict[str, Any], target_symbol: str) -> Path:
    return Path(config.get("paths", {}).get("reports_dir", "reports")) / target_symbol.upper() / "cross_asset_divergence_search.md"
...
def _candidate_id(row: pd.Series | dict[str, Any]) -> str:
    return "__".join(
        [
            ...
            f"mq{float(row['move_quantile']):g}",
            f"rq{float(row['recent_quantile']):g}",
            f"relq{float(row['relative_quantile']):g}",
...
def build_split_dataset(config: dict[str, Any], target_symbol: str) -> pd.DataFrame:
    ...
    output["feature_set"] = "cross_asset_divergence"
    output["n_states"] = 0
    output["seed"] = 0
    return output
...
def train_thresholds(train_frame: pd.DataFrame, spec: dict[str, Any], config: dict[str, Any]) -> dict[str, float]:
    cfg = _search_cfg(config)
    rel_q = float(spec["relative_quantile"])
    rel_low_q = 1.0 - rel_q
    thresholds = {
        "target_move_abs_min": _abs_quantile(train_frame, "target_dist_open", float(spec["move_quantile"])),
        "target_recent_abs_min": _abs_quantile(train_frame, "target_ret_3", float(spec["recent_quantile"])),
        ...
        "sector_rel_strength_low": _quantile(train_frame, "sector_rel_strength_count_2", float(cfg.get("breadth_low_quantile", 0.35)), -np.inf),
        "sector_rel_strength_high": _quantile(train_frame, "sector_rel_strength_count_2", float(cfg.get("breadth_high_quantile", 0.65)), np.inf),
        ...
        "equity_bonds_low": _quantile(train_frame, "spread_equity_bonds_12", float(cfg.get("credit_low_quantile", 0.35)), -np.inf),
        "equity_bonds_high": _quantile(train_frame, "spread_equity_bonds_12", float(cfg.get("credit_high_quantile", 0.65)), np.inf),
        ...
        "target_rv_rel_high": _quantile(train_frame, "target_rv_4_rel_by_bar", float(cfg.get("vol_high_quantile", 0.70)), np.inf),
        "qqq_spy_low": _quantile(train_frame, "relret_QQQ_SPY_6", rel_low_q, -np.inf),
        "qqq_spy_high": _quantile(train_frame, "relret_QQQ_SPY_6", rel_q, np.inf),
        "iwm_spy_low": _quantile(train_frame, "relret_IWM_SPY_6", rel_low_q, -np.inf),
        "iwm_spy_high": _quantile(train_frame, "relret_IWM_SPY_6", rel_q, np.inf),
        "dia_spy_low": _quantile(train_frame, "relret_DIA_SPY_6", rel_low_q, -np.inf),
        "dia_spy_high": _quantile(train_frame, "relret_DIA_SPY_6", rel_q, np.inf),
        "xlk_spy_high": _quantile(train_frame, "relret_XLK_SPY_6", rel_q, np.inf),
    }
    if not np.isfinite(thresholds["target_move_abs_min"]) or not np.isfinite(thresholds["target_recent_abs_min"]):
        return {}
...
def target_move_position(frame: pd.DataFrame, spec: dict[str, Any], thresholds: dict[str, Any]) -> pd.Series:
    dist_open = _clean_series(frame, "target_dist_open")
    recent = _clean_series(frame, "target_ret_3")
    move_min = float(thresholds["target_move_abs_min"])
    recent_min = float(thresholds["target_recent_abs_min"])
    active = _time_mask(frame, thresholds)
    short_mask = active & (dist_open.ge(move_min) | recent.ge(recent_min))
    long_mask = active & (dist_open.le(-move_min) | recent.le(-recent_min))
...
def _breadth_nonconfirm_mask(frame: pd.DataFrame, position: pd.Series, thresholds: dict[str, Any]) -> pd.Series:
    positive_index = _clean_series(frame, "positive_index_count_2")
    positive_sector = _clean_series(frame, "positive_sector_count_2")
    sector_rel = _clean_series(frame, "sector_rel_strength_count_2")
    ...
    upside_not_confirmed = (
        ...
        | sector_rel.le(float(thresholds.get("sector_rel_strength_high", np.inf)))
        | index_vwap.le(float(thresholds.get("index_above_vwap_high", np.inf)))
        | sector_vwap.le(float(thresholds.get("sector_above_vwap_high", np.inf)))
    )
    downside_not_confirmed = (
        ...
        | sector_rel.ge(float(thresholds.get("sector_rel_strength_low", -np.inf)))
        | index_vwap.ge(float(thresholds.get("index_above_vwap_low", -np.inf)))
        | sector_vwap.ge(float(thresholds.get("sector_above_vwap_low", -np.inf)))
    )
    return ((position < 0.0) & upside_not_confirmed) | ((position > 0.0) & downside_not_confirmed)


def _credit_nonconfirm_mask(frame: pd.DataFrame, position: pd.Series, thresholds: dict[str, Any]) -> pd.Series:
    ...
    upside_not_confirmed = (
        ...
        | equity_bonds.le(float(thresholds.get("equity_bonds_high", np.inf)))
    )
    downside_not_confirmed = (
        credit.ge(float(thresholds.get("credit_low", -np.inf)))
        | equity_bonds.ge(float(thresholds.get("equity_bonds_low", -np.inf)))
        | risk_off.le(float(thresholds.get("risk_off_high", np.inf)))
    )
    return ((position < 0.0) & upside_not_confirmed) | ((position > 0.0) & downside_not_confirmed)


def _relative_nonconfirm_mask(frame: pd.DataFrame, position: pd.Series, thresholds: dict[str, Any], target_symbol: str) -> pd.Series:
    target = target_symbol.upper()
    qqq_spy = _clean_series(frame, "relret_QQQ_SPY_6")
    iwm_spy = _clean_series(frame, "relret_IWM_SPY_6")
    dia_spy = _clean_series(frame, "relret_DIA_SPY_6")
    xlk_spy = _clean_series(frame, "relret_XLK_SPY_6")
    credit = _clean_series(frame, "spread_credit_12")

    if target == "QQQ":
        target_outperforms = qqq_spy.ge(float(thresholds.get("qqq_spy_high", np.inf)))
        target_underperforms = qqq_spy.le(float(thresholds.get("qqq_spy_low", -np.inf)))
        broad_not_up = (
            iwm_spy.le(float(thresholds.get("iwm_spy_high", np.inf)))
            | dia_spy.le(float(thresholds.get("dia_spy_high", np.inf)))
            | credit.le(float(thresholds.get("credit_high", np.inf)))
        )
        broad_not_down = (
            iwm_spy.ge(float(thresholds.get("iwm_spy_low", -np.inf)))
            | dia_spy.ge(float(thresholds.get("dia_spy_low", -np.inf)))
            | credit.ge(float(thresholds.get("credit_low", -np.inf)))
        )
        short_relative = target_outperforms & broad_not_up
        long_relative = target_underperforms & broad_not_down
    else:
        growth_led = qqq_spy.ge(float(thresholds.get("qqq_spy_high", np.inf))) | xlk_spy.ge(float(thresholds.get("xlk_spy_high", np.inf)))
        small_value_not_up = (
            iwm_spy.le(float(thresholds.get("iwm_spy_high", np.inf)))
            | dia_spy.le(float(thresholds.get("dia_spy_high", np.inf)))
            | credit.le(float(thresholds.get("credit_high", np.inf)))
        ...
        broad_not_down = (
            qqq_spy.ge(float(thresholds.get("qqq_spy_low", -np.inf)))
            | iwm_spy.ge(float(thresholds.get("iwm_spy_low", -np.inf)))
            | dia_spy.ge(float(thresholds.get("dia_spy_low", -np.inf)))
            | credit.ge(float(thresholds.get("credit_low", -np.inf)))
        )
        short_relative = growth_led & small_value_not_up
        long_relative = broad_not_down
    return ((position < 0.0) & short_relative) | ((position > 0.0) & long_relative)
...
def _vol_filter_mask(frame: pd.DataFrame, thresholds: dict[str, Any], vol_filter_name: str) -> pd.Series:
    ...
    if vol_filter_name == "exclude_vol_expansion":
    ...
    if vol_filter_name == "exclude_target_rv":
        return _clean_series(frame, "target_rv_4_rel_by_bar").lt(float(thresholds.get("target_rv_rel_high", np.inf)))
    raise ValueError(f"Unsupported volatility filter: {vol_filter_name}")


def divergence_position(
    frame: pd.DataFrame,
    spec: dict[str, Any],
    ...
    position = target_move_position(frame, spec, thresholds)
    active = position.abs() > 0.0
    if flags["breadth"]:
        active &= _breadth_nonconfirm_mask(frame, position, thresholds)
    if flags["credit"]:
        active &= _credit_nonconfirm_mask(frame, position, thresholds)
    if flags["relative"]:
        active &= _relative_nonconfirm_mask(frame, position, thresholds, target_symbol)
    active &= _vol_filter_mask(frame, thresholds, str(spec.get("vol_filter_name", "none")))
...
def _control_positions(
    ...
    alpha = divergence_position(frame, spec, thresholds, config, target_symbol)
    target_only = cap_trades_per_day(
        target_move_position(frame, spec, thresholds),
        frame,
...
def generate_candidate_specs(train_frame: pd.DataFrame, fold: int, horizon: int, config: dict[str, Any]) -> pd.DataFrame:
    ...
    for variant in [str(value) for value in cfg.get("variants", list(VARIANT_FLAGS))]:
        ...
        for side in [str(value) for value in cfg.get("sides", ["long", "short"])]:
            for move_q in [float(value) for value in cfg.get("move_quantiles", [0.55, 0.65])]:
                for recent_q in [float(value) for value in cfg.get("recent_quantiles", [0.55, 0.65])]:
                    for rel_q in [float(value) for value in cfg.get("relative_quantiles", [0.65, 0.75])]:
                        for vol_filter in [str(value) for value in cfg.get("vol_filters", ["none"])]:
                            spec = {
                                ...
                                "move_quantile": move_q,
                                "recent_quantile": recent_q,
                                "relative_quantile": rel_q,
...
def evaluate_candidate(
    ...
    positions = _control_positions(frame, spec_dict, thresholds, config, target_symbol, candidate_id, split)
    rows: list[dict[str, Any]] = []
    for bucket, position in positions.items():
        row = {
            **{key: spec_dict[key] for key in ["fold", "variant", "side", "horizon_bars", "move_quantile", "recent_quantile", "relative_quantile", "vol_filter_name"]},
            "split": split,
            "bucket": bucket,
...
def add_deltas(rows: pd.DataFrame) -> pd.DataFrame:
    ...
    signal["daily_sharpe_delta_vs_target_only"] = signal["daily_sharpe"] - signal["target_only_daily_sharpe"]
    signal["daily_sharpe_delta_vs_random"] = signal["daily_sharpe"] - signal["random_daily_sharpe"]
    signal["drawdown_reduction_vs_target_only"] = signal["target_only_max_drawdown"] - signal["max_drawdown"]
    signal["drawdown_reduction_vs_random"] = signal["random_max_drawdown"] - signal["max_drawdown"]
    delta_cols = [
        column
        for column in signal.columns
        if column.startswith(("target_only_", "random_", "inverted_", "net_delta_", "daily_sharpe_delta_", "drawdown_reduction_"))
    ]
...
def classify_validation_row(row: pd.Series, config: dict[str, Any]) -> str:
    ...
    if bool(cfg.get("require_target_only_improvement", True)):
        if float(row.get("net_delta_vs_target_only", -np.inf)) <= 0.0 and float(row.get("drawdown_reduction_vs_target_only", -np.inf)) <= 0.0:
            return "rejected_no_target_only_improvement"
    return "cross_asset_divergence_validation_candidate"
...
def validation_grid(dataset: pd.DataFrame, config: dict[str, Any], target_symbol: str) -> pd.DataFrame:
    cfg = _search_cfg(config)
    ...
    for (fold, horizon), validation_frame in validation.groupby(["fold", "horizon_bars"], sort=False):
        ...
        for _, spec in specs.iterrows():
            for scenario in scenarios:
                rows.append(evaluate_candidate(validation_frame.copy(), "validation", spec, scenario, config, target_symbol))
...
def select_specs(validation: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    candidates = validation[
        ...
        & validation["candidate_status"].eq("cross_asset_divergence_validation_candidate")
    ].copy()
    if candidates.empty:
    ...
    candidates["utility_score"] = (
        ...
        + candidates["net_delta_vs_target_only"].fillna(0.0)
...
def evaluate_selected_on_split(dataset: pd.DataFrame, specs: pd.DataFrame, split: str, config: dict[str, Any], target_symbol: str) -> pd.DataFrame:
    if specs.empty:
        return pd.DataFrame()
    ...
    for _, spec in specs.iterrows():
        ...
        if frame.empty:
            continue
        for scenario in scenarios:
            rows.append(evaluate_candidate(frame, split, spec, scenario, config, target_symbol))
    result = pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()
    return add_status(result, config) if not result.empty else result
...
def decision_table(validation: pd.DataFrame, test: pd.DataFrame, specs: pd.DataFrame, config: dict[str, Any]) -> pd.DataFrame:
    ...
    for _, spec in specs.iterrows():
        ...
        validation_ok = val_row.get("candidate_status", "") == "cross_asset_divergence_validation_candidate"
        ...
        rows.append(
            {
                **{col: spec[col] for col in ["candidate_id", "fold", "variant", "side", "horizon_bars", "vol_filter_name"]},
                ...
                "test_net_delta_vs_target_only_primary": primary_row.get("net_delta_vs_target_only", np.nan),
...
def render_report(
    ...
    family_summary = (
        ...
        .agg(
            ...
            median_net_delta_vs_target_only=("net_delta_vs_target_only", "median"),
    ...
    conclusion = (
        "At least one cross-asset divergence candidate passes the configured IBKR-aware gates."
        if not decisions.empty and decisions["decision"].eq("accepted_candidate").any()
        else "No cross-asset divergence candidate is accepted under the configured IBKR-aware gates."
    )
    return f"""# Cross-Asset Divergence Search - {target_symbol.upper()}
...
- Signal source: target directional move plus cross-asset non-confirmation.
...
def run(config_path: str | Path, target_symbol: str | None = None) -> tuple[Path, Path]:
    ...
    validation = validation_grid(dataset, config, target) if not dataset.empty else pd.DataFrame()
    ...
    test = evaluate_selected_on_split(dataset, specs, str(_search_cfg(config).get("test_split", "test")), config, target) if not specs.empty else pd.DataFrame()
    decisions = decision_table(selected_validation, test, specs, config)
    outputs = {
        "cross_asset_divergence_validation": results_dir / "cross_asset_divergence_validation.parquet",
        "cross_asset_divergence_selected_validation": results_dir / "cross_asset_divergence_selected_validation.parquet",
        "cross_asset_divergence_test": results_dir / "cross_asset_divergence_test.parquet",
        "cross_asset_divergence_selected_specs": results_dir / "cross_asset_divergence_selected_specs.parquet",
        "cross_asset_divergence_decisions": results_dir / "cross_asset_divergence_decisions.parquet",
    }
    results_dir.mkdir(parents=True, exist_ok=True)
    validation.to_parquet(outputs["cross_asset_divergence_validation"], index=False)
    selected_validation.to_parquet(outputs["cross_asset_divergence_selected_validation"], index=False)
    test.to_parquet(outputs["cross_asset_divergence_test"], index=False)
    specs.to_parquet(outputs["cross_asset_divergence_selected_specs"], index=False)
    decisions.to_parquet(outputs["cross_asset_divergence_decisions"], index=False)
    ...
    return report_path, outputs["cross_asset_divergence_decisions"]


def main() -> None:
    parser = argparse.ArgumentParser(description="Search train-calibrated cross-asset divergence candidates.")
    parser.add_argument("--config", default="configs/hmm_lab_15min_divergence.yaml")
    parser.add_argument("--target", default=None)
    args = parser.parse_args()

    report_path, decisions_path = run(args.config, args.target)
    print(f"Cross-asset divergence report written to: {report_path}")
    print(f"Cross-asset divergence decisions written to: {decisions_path}")
