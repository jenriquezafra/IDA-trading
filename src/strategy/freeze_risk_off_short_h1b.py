...
from src.strategy.risk_off_short_h1b_sweep import (
    DEFAULT_H1B_SWEEP_DIR,
    fit_h1b_filter,
)
from src.strategy.risk_off_short_triage import DEFAULT_HORIZON


DEFAULT_STRATEGY_SPEC_PATH = Path("configs/strategy/qqq_15min_risk_off_short_h1b_v1.yaml")
DEFAULT_FREEZE_DIR = DEFAULT_OUTPUT_DIR / "freeze_review" / "qqq_15min_risk_off_short_h1b_v1"


@dataclass(frozen=True)
class RiskOffH1BFreezeOutputs:
    output_dir: Path
...
def freeze_h1b_strategy(
    *,
    strategy_spec_path: str | Path = DEFAULT_STRATEGY_SPEC_PATH,
    h1b_sweep_dir: str | Path = DEFAULT_H1B_SWEEP_DIR,
...
) -> RiskOffH1BFreezeOutputs:
    spec_path = Path(strategy_spec_path)
    sweep_dir = Path(h1b_sweep_dir)
    ...
    selected_variant = _load_yaml(sweep_dir / "selected_variant.yaml")
    selected_decision = _load_yaml(sweep_dir / "selected_decision.yaml")
    ...
    frozen_decision = {
        ...
        "promotion_failed_gates": selected_decision.get("failed_gates", []),
    ...
    source_artifacts = {
        ...
        "h1b_report": sweep_dir / "report.md",
        "h1b_manifest": sweep_dir / "manifest.yaml",
        "h1b_validation_sweep": sweep_dir / "validation_sweep.parquet",
        "h1b_validation_gates": sweep_dir / "validation_gates.parquet",
        "h1b_selected_variant": sweep_dir / "selected_variant.yaml",
        "h1b_selected_trades": sweep_dir / "selected_trades.parquet",
        "h1b_selected_controls": sweep_dir / "selected_controls.parquet",
        "h1b_selected_concentration": sweep_dir / "selected_concentration.parquet",
        "h1b_selected_gates": sweep_dir / "selected_gates.parquet",
        "h1b_selected_decision": sweep_dir / "selected_decision.yaml",
    ...
    manifest = {
        ...
        "review": {
            "required_before_paper": [
                "robustness around q55/q45/credit q50",
                "higher-cost sensitivity",
                "subperiod and fold stability review",
                "StrategySpec implementation wired into runner",
            ]
        },
    }
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Freeze H1b risk-off short StrategySpec and artifacts")
    parser.add_argument("--strategy-spec", default=str(DEFAULT_STRATEGY_SPEC_PATH))
    parser.add_argument("--h1b-sweep-dir", default=str(DEFAULT_H1B_SWEEP_DIR))
    ...
    outputs = freeze_h1b_strategy(
        strategy_spec_path=args.strategy_spec,
        h1b_sweep_dir=args.h1b_sweep_dir,
